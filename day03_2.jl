...
symbols = findall(x -> x == '*', grid)
gears = symbols[map(length, get_adjacent_digits.(Ref(grid), symbols)) .== 2]  # 2 neighbors

numbers = []  # gear ratios
for gear in gears
    append!(numbers, prod(map(x -> part_number(grid, x), get_adjacent_digits(grid, gear))))
end

println("Part 2: ", sum(numbers))
